...
JOIN (SELECT ci.movie_id AS ci_movie_id FROM cast_info AS ci
 join name AS n on (ci.note IN ('(writer)',    '(head writer)',   '(written by)',  '(story)',  '(story editor)') AND n.gender != '' AND n.gender = 'f' AND n.id = ci.person_id ))S2 on (t.id = S2.ci_movie_id AND S2.ci_movie_id = mi_idx.movie_id AND S2.ci_movie_id = S1.mi_movie_id );