...
join aka_name AS an on (an.name != '' AND (an.name LIKE '%a%' OR an.name LIKE 'A%') AND an.person_id = S1.ci_person_id AND S1.n_id = an.person_id ) 
JOIN (SELECT pi.person_id AS pi_person_id FROM person_info AS pi
 join info_type AS it on (pi.note != '' AND it.info ='mini biography' AND it.id = pi.info_type_id ))S2 on (S2.pi_person_id = an.person_id AND S2.pi_person_id = S1.ci_person_id AND S1.n_id = S2.pi_person_id );