...
join movie_companies AS mc on (t.id = mc.movie_id AND mi_idx.movie_id = mc.movie_id ) 
...
join movie_info AS mi on (mi.info IN ('Horror', 'Action', 'Sci-Fi',  'Thriller',  'Crime', 'War') AND t.id = mi.movie_id AND mi.movie_id = mi_idx.movie_id AND mi.movie_id = mk.movie_id AND mi.movie_id = mc.movie_id ) 
