...
join keyword AS k on (k.keyword IN ('nerd', 'loner','alienation', 'dignity') AND k.id = mk.keyword_id ) 
join movie_info AS mi on (mi.note LIKE '%internet%' AND mi.info LIKE 'USA:% 200%' AND t.id = mi.movie_id AND mk.movie_id = mi.movie_id AND mi.movie_id = mc.movie_id AND mi.movie_id = cc.movie_id ) 
